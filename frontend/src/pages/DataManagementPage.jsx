...
const DataManagementPage = () => {
  ...
  const getFormFields = () => ({
    initiative: [
      { name: "name", label: "Initiative English Name", type: "text", required: true, columnSpan: 1 },
      { name: "arabic_name", label: "اسم المبادرة بالعربي", type: "text", required: true, columnSpan: 1, className: "text-right" },
      { name: "description", label: "Description in English", type: "textarea", required: true, columnSpan: 2 },
      { name: "arabic_description", label: "الوصف بالعربي", type: "textarea", required: true, columnSpan: 2, className: "text-right" }
    ],
    portfolio: [
      { name: "name", label: "Portfolio English Name", type: "text", required: true, columnSpan: 1 },
      { name: "arabic_name", label: "اسم المحفظة بالعربي", type: "text", required: true, columnSpan: 1, className: "text-right" },
      { 
  ...
  try {
    ...
    switch (activeTab) {
      case 'portfolios':
        ...
        // Convert portfolioManager to integer if exists
        if (data.portfolioManager) {
          data.portfolioManager = parseInt(data.portfolioManager, 10);
          if (isNaN(data.portfolioManager)) {
            toast.error('Invalid portfolio manager ID');
            throw new Error('Invalid portfolio manager ID');
          }
        }
        break;
      case 'initiatives':
    ...
    // Make API call with correct request body structure
    ...
    if(result.data.status === "success"){
      toast.success(`Successfully added ${getSingularTabName()}`);
      setShowForm(false);
