...
const RisksAndIssuesTable = ({ projectId, deliverables }) => {
  ...
  const addRisk = async () => {
    const response = await axiosInstance.post("/project-card/risk", {
      // {caseName, dueDate, comments, phaseName, linkedToType, linkedToId} variables need to pass
    });
  };

  const editRisk = async (riskData) => {
    try {
      const response = await axiosInstance.put(`/project-card/risk/${riskData.id}`, {
        caseName: riskData.riskName,
        phaseName: riskData.phaseName,
        responsePlan: riskData.responsePlan,
        status: riskData.riskStatus,
        linkedToType: riskData.projectId ? "Project" : "Deliverable",
        linkedToId: riskData.projectId || riskData.deliverableId,
      });
      console.log("Risk updated:", response.data);
      setTableData((prev) =>
        prev.map((risk) =>
          risk.id === riskData.id
            ? {
                ...
                responsePlan: riskData.responsePlan,
                status: riskData.riskStatus,
                projectId: riskData.projectId || null,
                deliverableId: riskData.deliverableId || null,
              }
            : risk
        )
      );
